...
function submitGuess() {
  ...
  const guess = activeTiles
    .map(tile => (tile.dataset.letter || '').toLowerCase().trim())
    .join('');
...
function flipTiles(tiles, guess) {
  const resultStates = Array(WORD_LENGTH).fill("wrong");
  ...
  for (let i = 0; i < WORD_LENGTH; i++) {
    if (guessLetters[i] === targetLetters[i]) {
      ...
      letterUsage[guessLetters[i]] = (letterUsage[guessLetters[i]] || 0) + 1;
    }
  }
  ...
  for (let i = 0; i < WORD_LENGTH; i++) {
    ...
    if (
      ...
      (letterUsage[letter] || 0) < targetLetterCounts[letter]
    ) {
      resultStates[i] = "wrong-location";
  ...
  tiles.forEach((tile, i) => {
    ...
    // Find the key on-screen, case-insensitive match
    const keys = keyboard.querySelectorAll("[data-key]");
    const key = Array.from(keys).find(k => k.dataset.key.toLowerCase() === letter);
    ...
    tile.addEventListener(
      ...
      () => {
        ...
        if (key) {
          key.classList.add(resultStates[i]);
        }
